...
### 8-1. 会話リセット機能
- [x] 「新しい会話」ボタンをヘッダーに追加する
- [x] ボタン押下でsessionIdを新規生成しlocalStorageを更新する
- [x] メッセージ一覧をクリアしてUI上も初期状態に戻す

### 8-2. エラーハンドリングの改善
- [x] APIエラーの種別（レート制限・タイムアウト・サーバーエラー）を判定する
- [x] エラー種別に応じたメッセージをUI上に表示する
- [x] 再試行ボタンを表示する

### 8-3. メッセージのコピーボタン
- [x] AIメッセージにホバーするとコピーボタンを表示する
- [x] コードブロックにコピーボタンを追加する（クリップボードにコピー）
- [x] コピー完了のフィードバック（「コピーしました」トースト等）を表示する

### 8-4. システムプロンプトのカスタマイズ
- [x] プリセット（汎用・翻訳・コードレビュー・要約）を定義する
- [x] ヘッダーにプリセット選択UIを追加する
- [x] 選択したプリセットをMastraエージェントの `instructions` に反映する

### 8-5. Vercel Function タイムアウト対策
- [x] `vercel.json` を作成して `/api/*` の `maxDuration` を60秒に設定する

### 8-6. 入力文字数制限・バリデーション
- [x] フロントエンドで入力を2000文字に制限する
- [x] 残り文字数カウンターを入力欄の下に表示する
- [x] バックエンドでも文字数を検証してエラーを返す

### 8-7. サイドバーで過去の会話一覧
- [x] `GET /api/conversations` エンドポイントを追加する（全セッション取得）
- [x] `DELETE /api/conversations/:sessionId` エンドポイントを追加する
- [x] サイドバーコンポーネントを実装する（会話一覧・選択・削除）
- [x] セッション切り替え時にメッセージ履歴を切り替える

### 8-8. レスポンシブ対応の強化
- [x] スマートフォンでキーボード表示時にフッターが隠れないよう修正する（`dvh` 単位使用）
- [x] サイドバーをモバイルではドロワーとして表示する
- [x] タッチ操作でのスクロールを改善する
